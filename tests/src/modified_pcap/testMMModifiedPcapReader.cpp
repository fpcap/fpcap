...
TEST(MMModifiedPcapReader, ConstructorSimple) {
    fpcap::MMModifiedPcapReader reader{"tracefiles/fritzbox-ip.pcap"};
...
TEST(MMModifiedPcapReader, ConstructorMissingFile) {
    EXPECT_THROW(fpcap::MMModifiedPcapReader{"missing-file"}, std::runtime_error);
}

TEST(MMModifiedPcapReader, FaultyConstructor) {
#ifdef __linux__ // throws un-catchable SEH exception on Windows
    EXPECT_THROW(fpcap::MMModifiedPcapReader{nullptr}, std::logic_error);
#endif
    EXPECT_THROW(fpcap::MMModifiedPcapReader{""}, std::runtime_error);
}
